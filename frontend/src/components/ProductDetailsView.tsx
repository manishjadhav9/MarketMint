...
export default function ProductDetailsView({ productId, onClose }: ProductDetailsViewProps) {
  ...
  const fetchProduct = async (id: string) => {
    try {
      setLoading(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'https://marketmint-backend.onrender.com/api'}/products/${id}`);
            ...
            src={
              product.images[activeImage]?.image_url
                ? product.images[activeImage].image_url.startsWith('http')
                  ? product.images[activeImage].image_url
                  : `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'https://marketmint-backend.onrender.com'}${product.images[activeImage].image_url}`
                ...
                <img
                  src={
                    img.image_url.startsWith('http')
                      ? img.image_url
                      : `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'https://marketmint-backend.onrender.com'}${img.image_url}`
                  }
